...
const UserView = (props: UserViewProps) => {
	...
	useEffect(() => {
		const URL = "https://hacker-news.firebaseio.com/v0/user/" + props.match.params.id + ".json";
		fetch(URL)
			...
			{user ? (
				<Paper variant="outlined" className={classes.paper}>
					<Grid container spacing={3}>
						<Grid item>
							<Avatar className={classes.karmaAvatar}>{user.karma}</Avatar>
						</Grid>
						<Grid item>
							<Typography variant="h4">{user.id}</Typography>
							<Typography variant="subtitle1" color="textSecondary">
								Created on {moment.unix(user.created).format("MMMM Do YYYY")}
								{" | "}
